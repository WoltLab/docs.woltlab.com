...
WoltLab Suite 5.4 differs between blocking direct contact only and hiding all contents when ignoring users.
To allow for detecting the difference `UserProfile::getIgnoredUsers()` and `UserProfile::isIgnoredUser()` methods received a new `$type` parameter.
